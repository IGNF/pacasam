...
- *Uncertainty Sampling* : on souhaite sélectionner des types de scènes sur lesquelles nos modèles de segmentation sémantique actuels (cf. [3d-deep-learning-library](https://anonymous.4open.science/r/3d-deep-learning-library/README.md)) manquent parfois de confiance, voire font des erreurs (p.ex. grands bâtiments).
...
- **Extractor**: créeent un jeu de données à partir d'un sampling.
    ...
    - `BDOrthoVintageExtractor` : extraction de patches d'orthoimages Infrarouge+RGB à partir de sources raster RGB et IRC PDAL-compatibles (format TIFF).
