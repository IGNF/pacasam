...
FROM
  (SELECT PATCHES_WITH_STATS_AND_CROISEMENT.*,
     ...
     (SELECT PATCHES_WITH_STATS.*,
      ...
      FROM
        (SELECT FICHIER_LIDAR_ID,
         ...
         JOIN STAT_VIGNETTE_LIDAR ON PATCHES.PATCH_ID = STAT_VIGNETTE_LIDAR.VIGNETTE_ID WHERE NB_TOTAL > 0) AS PATCHES_WITH_STATS
   ...
   JOIN FICHIER_LIDAR ON PATCHES_WITH_STATS_AND_CROISEMENT.FICHIER_LIDAR_ID = FICHIER_LIDAR.ID
