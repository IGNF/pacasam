...
*/ WITH FICHIER_LIDAR_REFERENCE AS
  (SELECT FICHIER_LIDAR.ID,
          ...
          EXTRACT(YEAR
                  FROM DATE_METADATA) AS LIDAR_YEAR_OF_ACQUISITION
   FROM FICHIER_LIDAR
  ...
  (SELECT DISTINCT ON (DALLE_ID,
                      ...
                      ANNEE AS ORHTOS_YEAR_OF_ACQUISITION
   FROM FICHIER_ORTHO
   WHERE CANAUX = 'RVB'),
        FICHIERS_IRC AS
  (SELECT DISTINCT ON (DALLE_ID,
  ...
  (SELECT FICHIERS_RGB.DALLE_ID,
          RGB_FILE,
          IRC_FILE,
          FICHIERS_RGB.ORHTOS_YEAR_OF_ACQUISITION
   FROM FICHIERS_RGB
   JOIN FICHIERS_IRC ON FICHIERS_RGB.DALLE_ID = FICHIERS_IRC.DALLE_ID
   AND FICHIERS_RGB.ORHTOS_YEAR_OF_ACQUISITION = FICHIERS_IRC.ORHTOS_YEAR_OF_ACQUISITION),
  ...
  (SELECT TEST_ET_EXCLUSION.TEST,
          ...
          DALLE.RACINE_DU_NOM || '-' || RIGHT('000000000' || CAST(VIGNETTES_AVEC_STATS_ET_CROISEMENT.VIGNETTE_ID AS VARCHAR),
                                          ...
                                          9) AS PATCH_ID,
          FICHIERS_ORTHOS_RGB_AND_IRC.*,
          VIGNETTES_AVEC_STATS_ET_CROISEMENT.*,
          BLOC.SRID,
          ROW_NUMBER() OVER (PARTITION BY VIGNETTES_AVEC_STATS_ET_CROISEMENT.VIGNETTE_ID
                             ORDER BY ABS(ORHTOS_YEAR_OF_ACQUISITION - LIDAR_YEAR_OF_ACQUISITION)) AS ORDER_OF_DIFF_YEARS_LIDAR_ORTHOS
