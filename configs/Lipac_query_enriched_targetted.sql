...
WITH FICHIER_LIDAR_REFERENCE AS
  ...
  (SELECT VIGNETTE_COLS.DALLE_ID,
          ...
          NB_SURSOL_PERENNE >= 10 AS NB_POINTS_SURSOL_PERENNE_HEQ_10,
          ((DENIVELE >= 30)
          ...
          (ALTITUDE >= 1000) AS POINTS_ALTITUDE_HEQ_1000M,
          ((NB_VEGETATION_HAUTE / (NB_TOTAL + 0.000001)) >= 0.9) AS FORET,
          ...
          CROISEMENT_VIGNETTE.PRESENCE_DE_SERRES,
          CROISEMENT_VIGNETTE.PRESENCE_DE_BATI_INDUSTRIEL_AGRICOLE_COMMERCIAL,
          CROISEMENT_VIGNETTE.PRESENCE_DE_BATI_SOUS_VEGET,
          CROISEMENT_VIGNETTE.PRESENCE_PARKING_VOITURES,
          CROISEMENT_VIGNETTE.PRESENCE_DE_PEAGES,
          CROISEMENT_VIGNETTE.PRESENCE_D_EOLIENNES,
          CROISEMENT_VIGNETTE.PRESENCE_D_ANTENNES,
          (CROISEMENT_VIGNETTE.PRESENCE_DE_SURFACES_D_EAU AND (NB_EAU >= 50)) AS PRESENCE_DE_SURFACES_D_EAU_ET_EAU,
          (CROISEMENT_VIGNETTE.PRESENCE_DE_PYLONES AND (NB_SURSOL_PERENNE >= 10)) AS PRESENCE_DE_PYLONE_ET_SURSOLPERENNE,
          (CROISEMENT_VIGNETTE.PRESENCE_D_ANTENNES AND (NB_SURSOL_PERENNE >= 10)) AS PRESENCE_D_ANTENNE_ET_SURSOLPERENNE,
          (CROISEMENT_VIGNETTE.PRESENCE_D_EOLIENNES AND (NB_SURSOL_PERENNE >= 10)) AS PRESENCE_D_EOLIENNE_ET_SURSOLPERENNE,
          (CROISEMENT_VIGNETTE.PRESENCE_DE_CATENAIRES AND (NB_SURSOL_PERENNE >= 10)) AS PRESENCE_DE_CATENAIRES_ET_SURSOLPERENNE,
          ((NB_SURSOL_PERENNE >= 10) AND ((NB_VEGETATION_HAUTE + NB_VEGETATION_MOYENNE) / (NB_TOTAL + 0.000001) >= 0.5)) AS PRESENCE_SURSOLPERENNE_ET_VEGETATION,
          CROISEMENT_VIGNETTE.PRESENCE_DE_CAMIONS,
          (CROISEMENT_VIGNETTE.PRESENCE_PARKING_VOITURES AND (NB_NON_CLASSES >= 250)) AS PRESENCE_PARKING_ET_VOITURES,
          (NB_NON_CLASSES >= 50) AS NB_POINTS_NON_CLASSES_HEQ_50,
          ((NB_EAU >= 50) AND (NB_SOL = 0)) AS POINTS_EAU_SANS_SOL,
          ((NB_EAU >= 50) AND (NB_SOL >= 100) AND (ALTITUDE BETWEEN -10 AND 10)) AS POINTS_EAU_ET_SOL_PROCHE_NIVEAU_MER
